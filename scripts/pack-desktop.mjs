...
const desktopDir = join(releaseRoot, 'PVPVECale-电脑版')
const templateDir = join(root, 'scripts', 'desktop-template')
const desktopZip = join(releaseRoot, 'PVPVECale-电脑版.zip')
...
cpSync(join(templateDir, '安装 PVPVECale.bat'), join(desktopDir, '安装 PVPVECale.bat'))
cpSync(join(templateDir, '安装 PVPVECale.vbs'), join(desktopDir, '安装 PVPVECale.vbs'))
cpSync(join(templateDir, '使用说明.txt'), join(desktopDir, '使用说明.txt'))

const setupExe = readdirSync(builderOut).find((name) => name.startsWith('PVPVECale-Setup-') && name.endsWith('.exe'))
if (setupExe) {
